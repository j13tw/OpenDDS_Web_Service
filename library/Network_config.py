...
#            print(line)
            if (x == 10):
                ...
                if (len(tmp[0].split("#")) == 1):
                    ...
                    self.cpu_short_load_status = "Disable"
                self.cpu_short_load = tmp[len(tmp)-1].split("\n")[0]
            if (x == 11):
                tmp = line.split(" ")
                if (len(tmp[0].split("#")) == 1):
                    self.cpu_middle_load_status = "Enable"
                else:
                    self.cpu_middle_load_status = "Disable"
            ...
            if (x == 12):
                ...
                if (len(tmp[0].split("#")) == 1):
                    ...
                    self.cpu_long_load_status = "Disable"
...
#        print(self.cpu_temperature)
    ...
    def start_cpu_load_short(self):
        command = "sudo sed -i '10c max-load-1 = " + \
            self_short_load + "' /etc/watchdog.conf"
    ...
    def start_cpu_load_middle(self):
        command = "sudo sed -i '11c max-load-5 = " + \
            self_middle_load + "' /etc/watchdog.conf"
        status = os.system(command)
        if (status == 0):
            return "OK"
        else:
    ...
    def start_cpu_load_long(self):
        command = "sudo sed -i '12c max-load-15 = " + \
            self_long_load + "' /etc/watchdog.conf"
        status = os.system(command)
        if (status == 0):
            return "OK"
        else:
    ...
    def set_cpu_temperature(self, temperature):
        command = "sudo sed -i '36c max-temperature = " + \
